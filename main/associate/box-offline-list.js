...
export default class BoxOfflineList extends Base {
  constructor(props) {
    super(props, {
      offlineLogs: null
    });
  ...
  getOfflineLogs() {
    // get offline list
    let query = BoxLog.query();
    query.order("id", false);
    BoxLog.exec(query).then( list => {
   ...
   */
  showOfflineLog(log) {
    this.navigator.push('boxOfflineLog', {log, _parent: this});
  }
      ...
      return (
        <Touchable onPress={() => this.showOfflineLog(log)} style={styles.offlineItemBox} key={"item" + log.data.id}>
          ...
          <View style={styles.offlineItemBody}>
            <Text style={styles.offlineItemBodyText}>待提交箱数 {log.data.nums}</Text>
          </View>
        </Touchable>
      );
    });
