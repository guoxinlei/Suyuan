...
export default class BoxOfflineLog extends Base {
  ...
  getOfflineLogs() {
    ...
    BoxLog.find(logId).then( log => {
      let product = {product_id: log.data.product_id, product_name: log.data.product}
      this.setState({boxLog: log, selectedProduct: product});
    }).catch( error => {
   ...
   */
  doDelete() {
    let boxLog = this.state.boxLog;
    boxLog.delete().then( result => {
      // refresh parent & pop
  ...
  associate() {
    ...
    BoxModel.exec(query).then( list => {
      list.map( box => {
        codelist[box.data.box_code] = box.data.case_code;
      });
      this.setState({ isLoading: true });

      Tools.post({
        url: Constants.api.associateBox,
        data: {productid: this.state.selectedProduct.product_id, boxcodes: codelist},
        success: (data) => {
    ...
    return (
      <View>
        ...
        <View style={[styles.row, {marginTop:13, borderTopWidth:1, borderTopColor:'#ddd'}]}>
          <Text style={styles.rowText}>待提交箱数      {this.state.boxLog.data.nums}</Text>
        </View>
